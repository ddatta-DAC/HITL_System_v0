...
class onlineUpdateExecutor:
    ...
    def __build__(
        ...
        # -------------------------------------------
        obj_list = []
        for i in tqdm(range(main_data_df.shape[0])):
            obj = record_class(main_data_df.iloc[i].to_dict(), _label=1, is_unserialized=True)
            obj_list.append(obj)
        ...
        self.classifier_obj = onlineUpdateExecutor.__get_trained_classifier__(X, y, num_domains, emb_dim)
        # Initial weights
        ...
        self.data_reference_df = data_reference_df.copy(deep=True)
        self.working_data = data_reference_df.copy(deep=True)
        # ---------------------
    ...
    def register_feedback_input(self, recordID, label, entity_pair_list = [], entity_list=[]):
        ...
        for entity_pair in entity_pair_list:
            ...
            dom_1 = entity_pair[0]
            dom_2 = entity_pair[1]
            entity_id_1 = self.data_handler_object.get_entity(self, recordID, dom_1)
            entity_id_2 = self.data_handler_object.get_entity(self, recordID, dom_2)
        ...
        self.data_reference_df.loc[self.working_data[ID_COL] == recordID, 'label'] = label
        return
    ...
    def __update_internal_model__(
        ...
        # Get predictions!
        # ------------------------------------------------------
        unmarked_recordIDs = self.working_data.loc[self.working_data['label']==0][ID_COL].values.to_list()
