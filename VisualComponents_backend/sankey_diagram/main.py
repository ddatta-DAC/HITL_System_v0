...
def obtain_data(
    ...
    hscode_value = tmp.loc[tmp['entity_ID'] == record['HSCode']]['value']

    if diagram_type == 1:
        _query_str = 'select {} from Records where HSCode={}'.format(
            ','.join(domain_subset), hscode_value
        )
...
def get_sankey_diagram(
    ...
    for col in list(df_1.columns):
        df_1[col] = df_1.parallel_apply( lambda x: label2id[x])
    ...
    for a,b in zip(domain_subset[:-1],domain_subset[1:]):
        _df_ = df_1[[a,b]].groupby([a,b]).count().reset_index(name="count")
